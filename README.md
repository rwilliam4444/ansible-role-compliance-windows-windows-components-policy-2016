...
# Description:
This Windows Components Policy Role was based off the CIS specs for 2016
servers.  This role covers the "Windows Components" CIS section only. The checks
and remediation commands are for local settings only. Group Policy settings may
override these settings. When the "remediate" variable is set to "YES", the role
will try to remediate the server's setting(s) accoridng to the CIS standards.  
The defaults/main.yml file can be used to disable specific CIS items (i.e.
...
# Role Variables:
# defaults file for ansible-role-compliance-windows-windows-components-policy-2016
Parameter | Choices/Defaults|Comments
----------|-----------------|--------
__remediate__ |"NO"| variable used to determine whether or not to remediate settings.
